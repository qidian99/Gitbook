---
description: >-
  NOTE: scrollmagic plugin animation.gsap.js is not working with React and GSAP
  b/c webpack and AMD module definition issue
---
...
{% embed url="https://greensock.com/react/" %}

{% embed url="https://greensock.com/scrollmagic/" %}

## Solving the GSAP Incompatibility

{% embed url="https://greensock.com/forums/topic/20434-react-gsap-scrollmagic-animationgsap-not-found/" %}
...
1. Find these files in "node\_module" directory tree \(may have different location on you PC\) and import it in this way to your working JS file \(App.js for example\). 
...
### My Solution: Tweak AMD definition
...
(function (root, factory) {
	if (typeof define === 'function' && define.amd) {
		...
		define(function (require) {
			...
			return factory(ScrollMagic, gsap, TweenMax, TimelineMax);
		});
	} else if (typeof exports === 'object') {
		// CommonJS
		// Loads whole gsap package onto global scope.
		var gsap = require("gsap") || require("gsap/dist/gsap");

		// TweenMax/TimelineMax will be global in v2. In v3, they will be on the gsap object
		factory(require('scrollmagic'), gsap, gsap.TweenMax || TweenMax, gsap.TimelineMax || TimelineMax);
	} else {
		// Browser globals
		factory(ScrollMagic || (jQuery && jQuery.ScrollMagic), gsap, gsap || TweenMax || TweenLite, gsap || TimelineMax || TimelineLite);
	}
}(this, function (ScrollMagic, Gsap, Tween, Timeline) {
	"use strict";
	var NAMESPACE = "animation.gsap";
	// If there's no version, we assume it's in ES6 environment and call the new API
	// var GSAP3_OR_GREATER = (Gsap && parseFloat(Gsap.version) >= 3) || !Gsap.version;
	...
	ScrollMagic.Scene.extend(function () {
		...
		Scene.setTween = function (TweenObject, duration, params) {
			...
			try {
				...
				log(1, "ERROR calling method 'setTween()': Supplied argument is not a valid TweenObject", e, Timeline, Gsap.version);
				return Scene;
			}
			if (_tween) { // kill old tween?
				Scene.removeTween();
